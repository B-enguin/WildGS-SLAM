...
class GUI:
    def __init__(self, q_map2vis: Queue):
        ...
        # IMAGE_SIZE = (int(IMAGE_SIZE[0] * 1.), int(IMAGE_SIZE[1] * 1.5))  # Resize to 75% of original size
        ...
        cv2.imshow('gt', self.gt)
        cv2.imshow('rendered', self.rendered)
        cv2.imshow('uncertainty', self.uncertainty)
        cv2.imshow('trajectory', self.traj_img)
        cv2.imshow('splat', self.splat)
        cv2.waitKey(1)
    ...
    def update_traj(self, new_traj):
        ...
        plot.traj(ax, plot_mode, traj, plot_start_end_markers=True)
        # ax.axis('equal')
        # plot.draw_coordinate_axes(ax, traj, plot_mode, 0.001)
        canvas.draw()
