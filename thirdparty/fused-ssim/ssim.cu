...
__global__ void fusedssimGoCUDA(
    int H,
    int W,
    int CH,
    float C1,
...
#pragma unroll
            ...
            if (pix_x < W && pix_y < H) {
                ...
                float sigma1_sigma2 = sqrt(max(sigma1_sq, 0.0)) * sqrt(max(sigma2_sq, 0.0));
                float L = (2 * mu1 * mu2 + C1) / (mu1_sq + mu2_sq + C1);
                float S = (2 * sigma1_sigma2 + C2) / (sigma1_sq + sigma2_sq + C2);
                float C = (sigma12 + (C2 / 2.0f)) / (sigma1_sigma2 + (C2 / 2.0f));
